...
function send(to: string, subject: string, html: string) {
  ...
  if (!key) return; // opt-in — no key, no send
  ...
  send(
    ...
    `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;color:#232323">
      ...
      <div style="background:#fff;padding:32px;border:1px solid #e8e5e0;border-top:none;border-radius:0 0 8px 8px">
        <p style="margin:0 0 8px;font-size:18px;font-weight:700">${period} is ready.</p>
        ...
        <a href="${opts.portalUrl}" style="display:inline-block;background:#0C0B0A;color:#fff;
        ...
        <p style="color:#9a9490;font-size:11px;margin:24px 0 0;line-height:1.6">
          Hathorn Advisory Group &nbsp;·&nbsp; ${opts.clientName}
        </p>
      </div>
    </div>`,
  );
...
export function sendCommentReply(opts: {
  ...
  send(
    ...
    `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;color:#232323">
      ...
      <div style="background:#fff;padding:32px;border:1px solid #e8e5e0;border-top:none;border-radius:0 0 8px 8px">
        <p style="margin:0 0 4px;font-size:13px;color:#9a9490;text-transform:uppercase;letter-spacing:.06em">${opts.metric}</p>
        <p style="margin:0 0 16px;font-size:16px;font-weight:700">${opts.advisorName} replied</p>
        <div style="background:#f4f3f0;border-radius:8px;padding:16px;font-size:14px;line-height:1.6;margin-bottom:24px">
          ${opts.body}
        </div>
        <a href="${opts.portalUrl}" style="display:inline-block;background:#0C0B0A;color:#fff;
          font-size:13px;font-weight:700;padding:12px 24px;border-radius:8px;text-decoration:none">
