...
const SECRET = new TextEncoder().encode(process.env.AUTH_SECRET || "dev-secret-change-in-production-9f2a");
...
export async function middleware(req: NextRequest) {
  ...
  if (!token) return deny(req, isApi, 401);
...
export const config = {
  matcher: [
    "/admin/:path*", "/dash/:path*", "/today/:path*", "/portfolio/:path*", "/clients/:path*", "/engagement/:path*", "/review/:path*", "/upload/:path*", "/portal/:path*",
